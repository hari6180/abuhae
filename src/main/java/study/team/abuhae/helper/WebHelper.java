...
public class WebHelper {
	...
	public String getUploadUrl(String filePath) {
		//URL상의 업로드 폴더와 파일이름을 결합하여 파일 객체 생성
		File f = new File(this.uploadPath, filePath);
		//결합된 경로 추출
		String path = f.getPath();
		//window의 경우 경고 구문을 역슬래시로 하는데, 이는 웹에 노출 할수있는 형태가 아니므로 
	...
	NullPointerException, Exception {
		...
		File targetFile = null;   // 저장된 파일 정보를 담기 윟나 File 객체
		int count = 0;            // 중복된 파일 수
		
		//일단 무한루프
		while(true) {
			// 저장된 파일 이름 --> 현재시각 + 카운트값 + 확장자
	 ...
	 * @throws Exception
	 */
	public String createTuhmbnail(String path, int width, int height, boolean crop) throws Exception{
